...
export const BUSINESS = {
  ...
  // Distance rules, measured by real driving distance from this address —
  // your home base. Both need GOOGLE_MAPS_API_KEY set (same key used for
  // drive-time buffering) to actually take effect; if it's not set, neither
  // rule is enforced and everything behaves as if every property is close.
  originAddress: "8047 Fall River Dr, Dallas, TX",
  // Properties farther than this can't be booked with a start time that
  // would run past closing — no "start right at close and run over"
  // exception for them, unlike closer properties.
  strictCloseDistanceMiles: 45,
  // Properties farther than this get a flat trip charge added to the price.
  tripChargeDistanceMiles: 50,
  tripChargeAmount: 30,
  // If a new shoot's property is farther than this from ANY shoot already
  // booked that same day, that whole day is excluded — the client has to
  // pick a different day rather than squeeze in a long cross-town drive.
  // This is separate from the drive-time buffer, which only spaces shoots
  // out in time; this rule refuses same-day entirely past this distance.
  maxSameDayDistanceMiles: 40,
};
...
export const ADDONS = [
  ...
  {
    id: "homewebsite",
    name: "Custom Home Website",
    description: "MLS-approved single-property tour link.",
    extraMinutes: 0,
    price: 25,
  },
];
