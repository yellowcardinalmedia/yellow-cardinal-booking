...
import { driveTimeMinutes, driveDistanceMiles } from "./maps";
...
export async function generateSlotsForDay(dateStr, durationMinutes, busyEvents, newAddress, strictClose = false) {
  ...
  // a shoot booked right at close is allowed to run past it (unless strictClose).
  ...
  // If any shoot already booked this day is too far from the new address,
  // exclude the whole day rather than trying to squeeze it in with just a
  // time buffer — a 40+ mile cross-town drive isn't worth it regardless of
  // how much time is technically between the two appointments.
  if (newAddress) {
    const knownLocations = [...new Set(busy.map((b) => b.location).filter(Boolean))];
    console.log(`[same-day check] date=${dateStr} newAddress="${newAddress}" knownLocations=`, knownLocations);
    for (const loc of knownLocations) {
      const miles = await driveDistanceMiles(loc, newAddress);
      console.log(`[same-day check] "${loc}" -> "${newAddress}" = ${miles} miles (threshold ${BUSINESS.maxSameDayDistanceMiles})`);
      if (miles !== null && miles > BUSINESS.maxSameDayDistanceMiles) {
        console.log(`[same-day check] EXCLUDING ${dateStr} — over threshold`);
        return [];
      }
    }
  }
