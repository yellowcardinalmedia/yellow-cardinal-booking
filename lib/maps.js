// Looks up real drive time between two addresses using the Google Maps
// Distance Matrix API. Requires GOOGLE_MAPS_API_KEY (separate from the
// Calendar/Sheets OAuth credentials — this one's a simple API key).
// Falls back to null (caller uses a flat buffer instead) if the key is
// missing or the lookup fails, so the app degrades gracefully.
...
// away to trigger a trip charge / earlier cutoff" rules. Returns null (rule
// is skipped, not enforced) if the key is missing or the lookup fails.
